...
async function initDBInternal() {
  ...
  `);
    ...
    sqlDb.run(`CREATE TABLE IF NOT EXISTS agent_assignments (
    ...
    for (const id of AGENT_IDS) {
        ...
        if (Number(c) === 0) {
            sqlDb.run(`INSERT INTO agent_assignments (agent_id, enabled, for_management, for_interactions)
    ...
    sqlDb.run(`CREATE TABLE IF NOT EXISTS admin_phones (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    sqlDb.run(`CREATE TABLE IF NOT EXISTS wx_create_state (
    ...
    if (adminPhone) {
        try {
            sqlDb.run(`INSERT OR IGNORE INTO admin_phones (phone, label) VALUES (?, 'owner')`, [adminPhone]);
        }
        catch { /* ignore */ }
    }
    ...
    try {
        sqlDb.run('ALTER TABLE users ADD COLUMN is_tester INTEGER DEFAULT 0');
    }
    catch { /* exists */ }
    try {
        sqlDb.run('ALTER TABLE users ADD COLUMN free_passes INTEGER DEFAULT 0');
    }
    catch { /* exists */ }
...
export function addAdminPhone(phone, label = 'operator') {
    const p = (phone || '').replace(/[^\d]/g, '');
    if (!p)
        return false;
    try {
...
export function isFreeUser(phone) {
    const p = (phone || '').replace(/[^\d]/g, '');
    if (!p)
        return false;
    const row = db.prepare('SELECT is_tester, free_passes FROM users WHERE phone = ?').get(p);
    if (!row)
        return false;
    return row.is_tester === 1 || (Number(row.free_passes) || 0) > 0;
}
export function grantPasses(phone, n) {
    const p = (phone || '').replace(/[^\d]/g, '');
    if (!p || !(Number(n) > 0))
        return false;
    try {
...
export function consumeFreePass(phone) {
    const p = (phone || '').replace(/[^\d]/g, '');
    if (!p)
        return false;
    const row = db.prepare('SELECT id, is_tester, free_passes FROM users WHERE phone = ?').get(p);
    if (!row)
        return false;
    if (row.is_tester === 1)
        return true;
