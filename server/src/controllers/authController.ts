...
export function getMe(req: AuthRequest, res: Response) {
  ...
  res.json({
    ...
    firstName: (req.user as any).first_name || null,
    lastName: (req.user as any).last_name || null,
    avatarUrl: (req.user as any).avatar_url || null,
...
export function updatePreferences(req: AuthRequest, res: Response) {
  if (!req.user) return res.status(401).json({ error: 'Not authenticated' });
...
export function deleteAccount(req: AuthRequest, res: Response) {
  if (!req.user) return res.status(401).json({ error: 'Not authenticated' });
...
export async function verifyOTP(req: AuthRequest, res: Response) {
  ...
  let user = db.prepare('SELECT * FROM users WHERE phone = ?').get(normalized) as any;
