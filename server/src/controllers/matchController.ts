...
export async function confirmResult(req: AuthRequest, res: Response) {
  ...
  try {
    const tournament = db.prepare('SELECT name FROM tournaments WHERE id = ?').get(match.tournament_id) as any;
    const p1 = db.prepare('SELECT username FROM users WHERE id = ?').get(match.player1_id) as any;
    const p2 = db.prepare('SELECT username FROM users WHERE id = ?').get(match.player2_id) as any;
