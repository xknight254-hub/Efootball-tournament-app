    ...
    <div className="space-y-6">
      ...
      {activeTab === 'bracket' && (
        <div className="rounded-2xl p-6 overflow-x-auto" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)' }}>
          <h3 className="text-base font-semibold text-white mb-5">Tournament Bracket</h3>
          {matches.length === 0 ? (
            <div className="text-center text-[var(--color-text-muted)] py-12">No matches scheduled yet.</div>
          ) : (
                  ...
                  <div key={round} className="space-y-3">
                    ...
                    {roundMatches.map(match => (
                      <div key={match.id} className="match-card w-56">
                        <div className={`flex justify-between items-center py-1.5 ${match.winner?.id === match.player1?.id ? 'text-[#22c55e]' : match.winner?.id && match.winner?.id !== match.player1?.id ? 'text-[var(--color-text-dim)]' : 'text-white'}`}>
                          <span className="text-xs font-medium truncate max-w-[100px]">{match.player1?.username || 'TBD'}</span>
                          <span className="font-mono text-xs">{match.player1Score ?? '-'}</span>
                        </div>
                        <div className="h-px" style={{ background: 'var(--color-border-subtle)' }} />
                        <div className={`flex justify-between items-center py-1.5 ${match.winner?.id === match.player2?.id ? 'text-[#22c55e]' : match.winner?.id && match.winner?.id !== match.player2?.id ? 'text-[var(--color-text-dim)]' : 'text-white'}`}>
                          <span className="text-xs font-medium truncate max-w-[100px]">{match.player2?.username || 'TBD'}</span>
                          <span className="font-mono text-xs">{match.player2Score ?? '-'}</span>
                        </div>
                        {match.status !== 'completed' && isAuthenticated() && (
                          <div className="flex gap-1.5 mt-2 pt-2" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
                            ...
                            {match.status === 'playing' && (
                              <>
                                <button onClick={() => api.matches.confirm(match.id).then(loadData)} className="flex-1 text-xs py-1 rounded" style={{ background: 'rgba(34,197,94,0.1)', color: '#4ade80' }}>Confirm</button>
                                <button onClick={() => api.matches.dispute(match.id).then(loadData)} className="flex-1 text-xs py-1 rounded" style={{ background: 'rgba(245,158,11,0.1)', color: '#fbbf24' }}>Dispute</button>
                              </>
                            )}
                          </div>
                        )}
                        {match.status === 'completed' && <div className="mt-2 text-xs text-center text-[#22c55e] font-medium">✓ Completed</div>}
      ...
      {activeTab === 'standings' && (
        <div className="rounded-2xl p-5" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)' }}>
          ...
          {tournament.format !== 'league' ? <div className="text-center text-[var(--color-text-muted)] py-8">Standings available for league format only.</div> : participants.length === 0 ? <div className="text-center text-[var(--color-text-muted)] py-8">No participants yet.</div> : (
            <table className="data-table">
              <thead><tr><th>#</th><th>Player</th><th className="text-center">P</th><th className="text-center">W</th><th className="text-center">D</th><th className="text-center">L</th><th className="text-center">Pts</th></tr></thead>
              <tbody>{participants.map((p, i) => (
                <tr key={p.id}>
                  <td className="text-[var(--color-text-muted)]">{i + 1}</td>
                  <td><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>{p.username.charAt(0).toUpperCase()}</div><span className="text-white text-sm">{p.username}</span></div></td>
                  <td className="text-center text-[var(--color-text-secondary)]">0</td><td className="text-center text-[var(--color-text-secondary)]">0</td><td className="text-center text-[var(--color-text-secondary)]">0</td><td className="text-center text-[var(--color-text-secondary)]">0</td>
                  <td className="text-center text-[#22c55e] font-bold">0</td>
                </tr>
              ))}</tbody>
            </table>
          )}
